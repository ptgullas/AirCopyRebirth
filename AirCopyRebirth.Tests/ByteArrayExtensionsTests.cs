...
namespace AirCopyRebirth.Tests {
    public class ByteArrayExtensionsTests {
        ...
        [Fact]
        public void Contains_ReadScanPreviewWithEndTagFromFileStreamEndWhenReadEqualsZero_ReturnsTrue() {
        ...
        [Fact]
        public void Contains_ReadScanPreviewWithEndTagFromFileStreamEndWhenReadEqualsZeroOrEndTagIsFound_ReturnsTrue() {
            string pathToPreview = @"..\..\..\SampleData\previewWithEndMarker.jpg";
            // quick way to test the path to the Sample Data
            // Assert.True(File.Exists(pathToPreview));
            ...
            using (FileStream fs = new FileStream(pathToPreview, FileMode.Open)) {
                ...
                while ((bytesReadThisLoop = fs.Read(buffer, totalBytesRead, bytesToReadPerLoop)) > 0 && foundPattern == false) {
                    totalBytesRead += bytesReadThisLoop;

                    // check for end tag in buffer minus the # of bytes read this loop minus the tag length
                    // in case the end tag is torn apart
                    int offSetToCheck = totalBytesRead - (bytesReadThisLoop - endMarker.Length - 1);
                    if (offSetToCheck < 0) {
                        offSetToCheck = 0;
                    }
                    if (buffer.Contains(endMarker, offSetToCheck)) {
                        foundPattern = true;
                    }
                }
                Console.Write("out of while loop\n");
